	...
	for(ind_arg=1;ind_arg<(argc);ind_arg++)
	{
	        if(argv[ind_arg][0] == '-')
	        {
			  ...
			  ind_arg++;
	      	  ...
	      	  {
	      		  T[0]=atof(argv[ind_arg+1]);
	      		  xcenter=false;
			  ind_arg++;
	      	  }
	      	  else if(strcmp(argv[ind_arg]+1,"y")==0)//y movement
	      	  {
	      		  T[1]=atof(argv[ind_arg+1]);
	      		  ycenter=false;
			  ind_arg++;
	      	  }
	      	  else if(strcmp(argv[ind_arg]+1,"z")==0)//z movement
	      	  {
	      		  T[2]=atof(argv[ind_arg+1]);
	      		  zcenter=false;
			  ind_arg++;
	      	  }
	      	  else if(strcmp(argv[ind_arg]+1,"zind")==0)//z atom index to center
	      	  {
	      		  ind[2]=atoi(argv[ind_arg+1]);
	      		  zcenter=false;
			  ind_arg++;
	      	  }
	      	  else//Wrong Option
...
//Input-Output control
	...
	if((ind[2]!=0)&&(T[2]!=0))
	{
		cerr<<"ERROR: both explicit translation and atom to center along z "<<fileout<<endl;
		return 1;
	}
...
//TRANSLATION 	BEGIN
	...
	if(T[2]==0)T[2]=-inpdb[(ind[2])].z;
	if(xcenter)T[0]=-(inpdb.xcenter);
	if(ycenter)T[1]=-(inpdb.ycenter);
	if(zcenter)T[2]=-(inpdb.zcenter);
	inpdb.translate(T);
	file_out<<inpdb;
//TRANSLATION 	END
//---------------------------------------------------------
