	...
	for(ind_arg=1;ind_arg<(argc);ind_arg++)
	{
	        if(argv[ind_arg][0] == '-')
	        {
			  ...
			  ind_arg++;
	      	  ...
	      	  else if(strcmp(argv[ind_arg]+1,"ResStr")==0)//First Residue with 0 charge
	      	  {
			  res_str=atoi(argv[ind_arg+1]);
			  ind_arg++;
	      	  }
	      	  else if(strcmp(argv[ind_arg]+1,"ResEnd")==0)//Last Residue with 0 charge
	      	  {
...
//ADDING RADII AND CHARGE PARMETERS 	BEGIN
	for(ind_atm=0;ind_atm<inpdb.num_atm;ind_atm++){
		if((res_str!=0)||(res_last!=0)){
			if( (inpdb[ind_atm].res_num>=res_str)&&
				(inpdb[ind_atm].res_num<=res_last) )inpdb[ind_atm].chg=0.0;
		} else {
			inpdb.mol[ind_atm].chg=0.0;
		}
	}
	file_out<<inpdb;
